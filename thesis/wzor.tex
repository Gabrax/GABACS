...
Dla każdej z kamer przeprowadzana jest osobna kalibracja przy użyciu funkcji \texttt{cv2.calibrateCamera}. Wynikiem są:
...
\captionsetup{font=footnotesize}
\caption[Rezultat mapy dysparycji StereoSGBM i StereoBM. Opracowanie własne.]{Rezultat mapy dysparycji uzyskanej przy użyciu algorytmów StereoSGBM i StereoBM.}
\label{fig:disparity}
\end{figure}
...
Powyższy fragment kodu odpowiada za ograniczenie obszaru analizy binarnej maski \texttt{close\_mask} do wcześniej zdefiniowanego regionu zainteresowania. Uzyskana maska \texttt{roi\_mask} zawiera wartości binarne (0 i 255) tylko w wyznaczonym regionie zainteresowania, natomiast wszystkie pozostałe piksele przyjmują wartość 0. W ten sposób dalsza analiza (np. wykrywanie konturów, obliczanie miar) jest ograniczona wyłącznie do fragmentu obrazu istotnego z punktu widzenia aplikacji.
...
\begin{enumerate}
  ...
  Funkcja \texttt{findContours} wyszukuje kontury w binarnej masce \texttt{roi\_mask}, uprzednio ograniczonej do regionu zainteresowania. Zastosowany tryb \texttt{RETR\_EXTERNAL} powoduje, że wykrywane są wyłącznie kontury zewnętrzne, natomiast metoda \texttt{CHAIN\_APPROX\_SIMPLE} dokonuje kompresji punktów konturu w celu redukcji pamięci.
...
Dodatkowo, stereowizja napotyka trudności w przypadku dynamicznych scen, gdy obiekty poruszają się szybko, co może powodować artefakty w mapie głębi. Powtarzalne lub symetryczne wzory, jak kratki czy płytki, generują tzw. problemy ambiguitetowe, utrudniając jednoznaczne dopasowanie pikseli. Wysokie odległości między kamerą a obiektem ograniczają dokładność pomiaru, ponieważ dysparycja staje się bardzo mała. Ponadto niska jakość kamer, obecność szumu, czy złe warunki atmosferyczne, takie jak mgła, deszcz czy pył, mogą dodatkowo pogarszać jakość danych. Istotnym czynnikiem ograniczającym praktyczne zastosowania stereowizji jest także złożoność obliczeniowa algorytmów dopasowania dysparycji, co w klasycznych jednostkach CPU ogranicza liczbę przetwarzanych klatek na sekundę. Analiza wykazała, że osiągnięcie wartości około $4\,\text{FPS}$ jest niewystarczające do zapewnienia szybkiej i niezawodnej reakcji systemu. Dla porównania, przy zastosowaniu wydajniejszego CPU uzyskano około $16\,\text{FPS}$, co nadal nie spełnia wymagań systemu czasu rzeczywistego. Oznacza to, że dalsze zwiększanie mocy CPU nie jest wystarczającym rozwiązaniem.
