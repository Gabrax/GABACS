...
RL, RR, PL, PR, Q, roiL, roiR= cv2.stereoRectify(MLS, dLS, MRS, dRS, ChessImaR.shape[::-1], R, T, 0, (0,0))  # last paramater is alpha, if 0= croped, if 1= not croped
...
while True:
    ...
    left_nice  = executor.submit(cv2.remap, left_frame, Left_Stereo_Map[0], Left_Stereo_Map[1],interpolation=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT)
    right_nice = executor.submit(cv2.remap, right_frame, Right_Stereo_Map[0], Right_Stereo_Map[1],interpolation=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT)
    Left_rect  = left_nice.result()
    Right_rect = right_nice.result()

    small_left  = executor.submit(lambda: cv2.resize(Left_rect, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA))
    small_right = executor.submit(lambda: cv2.resize(Right_rect, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA))
