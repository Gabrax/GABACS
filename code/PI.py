...
for i in range(0, 64):
    ...
    if ChessImaR is None or ChessImaL is None:
        print(f"âš ď¸Ź Warning: Image {t} could not be loaded.")
...
while True:
    ...
    Left_remap = executor.submit(cv2.remap, left_frame, Left_Stereo_Map[0], Left_Stereo_Map[1],
                                interpolation=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT)
    Right_remap = executor.submit(cv2.remap, right_frame, Right_Stereo_Map[0], Right_Stereo_Map[1],
                                 interpolation=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT)
    Left_nice = Left_remap.result()
    Right_nice = Right_remap.result()
    
    # Resize disparity maps
    small_left  = executor.submit(lambda: cv2.resize(Left_nice, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA))
    small_right = executor.submit(lambda: cv2.resize(Right_nice, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA))
    Left_small  = small_left.result()
    Right_small = small_right.result() 

    # Convert to grayscale
    Left_gray = executor.submit(cv2.cvtColor, Left_small, cv2.COLOR_BGR2GRAY)
    Right_gray = executor.submit(cv2.cvtColor, Right_small, cv2.COLOR_BGR2GRAY)
    gray_left = Left_gray.result()
    gray_right = Right_gray.result()

    # Compute disparity maps
    Left_disparity = executor.submit(lambda: stereo.compute(gray_left, gray_right).astype(np.float32) / 16.0)
    Right_disparity = executor.submit(lambda: stereoR.compute(gray_right, gray_left).astype(np.float32) / 16.0)
    dispL = Left_disparity.result()
    dispR = Right_disparity.result()
    ...
    cv2.putText(disp_closed, f"Size: {disp_width}x{disp_height}", (10, 50),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
